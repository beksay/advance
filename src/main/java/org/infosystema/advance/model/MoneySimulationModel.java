package org.infosystema.advance.model;

import java.util.List;

import org.infosystema.advance.beans.FilterExample;
import org.infosystema.advance.domain.MoneySimulation;
import org.infosystema.advance.service.MoneySimulationService;

/**
 * 
 * @author Kuttubek Aidaraliev
 *
 */

public class MoneySimulationModel extends BaseModel<MoneySimulationService, MoneySimulation, Integer> {

	private static final long serialVersionUID = -4871106869905562775L;

	public MoneySimulationModel(List<FilterExample> filters, MoneySimulationService service) {
		super(filters, service);
	}
	
	@Override
	protected Integer getKey(String key) {
		return Integer.parseInt(key);
	}
	
}
